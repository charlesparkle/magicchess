...
function initRealtimeComments(postId) {
  ...
  function connect() {
    ...
    realtimeSocket.onclose = () => {
      ...
      // FIX: On reconnect, re-fetch missed comments and register them in
      // renderedCommentIds BEFORE re-opening the WebSocket. This prevents
      // a race condition where the REST response and the first WS INSERT
      // event arrive simultaneously and the Set check is bypassed.
      setTimeout(async () => {
        if (document.visibilityState === 'hidden') return;
        try {
          const missed = await fetchComments(postId);
          if (Array.isArray(missed)) {
            missed.forEach(c => {
              if (!renderedCommentIds.has(c.id)) {
                appendComment(c, false); // no animation for catch-up comments
              }
            });
          }
        } catch {
          // Best-effort — ignore fetch errors during reconnect
        }
        connect();
      }, 3000);
    };
...
function buildCommentEl(c, animate = false) {
  ...
  div.setAttribute('aria-label', `Komentar dari ${sanitize(c.author)}`);
  div.innerHTML = `
    <div class="comment-item__avatar" style="background:${sanitize(c.author_color ?? '#6366f1')}" aria-hidden="true">
      ${sanitize(c.author_initials ?? '?')}
    </div>
    <div class="comment-item__body">
      <div class="comment-item__header">
        <span class="comment-item__author">${sanitize(c.author)}</span>
        <time class="comment-item__time" datetime="${c.created_at}">${timeAgo(c.created_at)}</time>
      </div>
...
// Cache untuk mencegah fetch duplikat
let _postCache = new Map();
let _relatedCache = new Map();

async function fetchPost(id) {
  // Validasi ID
  const postId = String(id).trim();
  if (!postId || (!USE_MOCK && !/^\d+$/.test(postId))) {
    throw new Error('ID post tidak valid');
  }
  
  // Check cache first
  const cacheKey = `post_${postId}`;
  if (_postCache.has(cacheKey)) {
    return _postCache.get(cacheKey);
  }
  
  let post;
  if (USE_MOCK) {
    post = MOCK_POSTS.find(p => p.id === Number(postId));
    if (!post) throw new Error('Post tidak ditemukan');
  } else {
    const rows = await supabaseGet('posts', `id=eq.${encodeURIComponent(postId)}&select=*&limit=1`);
    if (!rows.length) throw new Error('Post tidak ditemukan');
    post = rows[0];
  }
  
  // Cache hasil
  _postCache.set(cacheKey, post);
  // Clear cache setelah 5 menit
  setTimeout(() => _postCache.delete(cacheKey), 5 * 60 * 1000);
...
async function fetchRelated(category, excludeId) {
  // Check cache
  const cacheKey = `related_${category}_${excludeId}`;
  ...
  let related;
  if (USE_MOCK) {
    related = MOCK_POSTS.filter(p => p.category === category && p.id !== Number(excludeId)).slice(0, 4);
  } else {
    related = await supabaseGet(
      'posts',
  ...
  setTimeout(() => _relatedCache.delete(cacheKey), 2 * 60 * 1000);
...
async function renderPost(post) {
  ...
  if (post.body) {
    ...
    bodyEl.innerHTML = purify.sanitize(post.body, {
      ALLOWED_TAGS: [
        ...
        'a','br','hr','img','div','span',
      ],
      ALLOWED_ATTR: ['href','target','rel','src','alt','class','style','loading','title'],
      ALLOW_DATA_ATTR: false,
    });

    // Post-process: fix lineup hero grid — ensure all flex containers stay row-wrap
    bodyEl.querySelectorAll('div[style*="flex"]').forEach(div => {
      const style = div.getAttribute('style') || '';
      // If it has images inside and looks like a hero grid, enforce row wrap
  ...
  // --- TAMBAHKAN KODE INI UNTUK MERENDER GAMBAR/LAMPIRAN ---
  if (post.attachment_url) {
    const url = sanitize(post.attachment_url);
    // Jika link berupa gambar (jpg/png/gif/webp/avif)
    if (url.match(/\.(jpeg|jpg|gif|png|webp|avif)(\?.*)?$/i)) {
      bodyEl.innerHTML += `
        <div style="margin-top: 24px; border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.1);">
          <img src="${url}" alt="Attachment" style="width: 100%; display: block; object-fit: cover;">
        </div>`;
    } 
    // Jika link berupa video mp4
    ...
    // Jika tautan biasa (PDF, YouTube, dll)
    else {
      bodyEl.innerHTML += `
        <div style="margin-top: 24px;">
          <a href="${url}" target="_blank" rel="noopener noreferrer" class="btn btn--primary" style="display: inline-flex;">
            <i class="ph-bold ph-link"></i> Buka Lampiran
          </a>
        </div>`;
    }
  }
...
async function _doSubmitComment(postId, input, submitBtn, charCount) {
  ...
  if (!modResult.ok) {
    showToast('⚠ ' + modResult.errors[0]);
    // Flash merah pada textarea sebagai visual feedback
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Fetch post + komentar secara paralel, lalu fetch related menggunakan kategori dari post
  try {
    ...
    // Fetch related posts using category from already-fetched post (no duplicate fetch)
    const related = await fetchRelated(post.category, id).catch(() => []);
