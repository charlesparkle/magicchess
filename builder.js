/**
 * builder.js — Lineup Builder Engine (ES Module)
 * All hero/equipment/synergy data imported from hero-db.js.
 */
import {
  HERO_DB,
  EQUIP_POOL,
...
import { initAuth, requireAuth, onAuthChange } from './auth.js';
...
function renderSynergyDisplay(tp, mortalActive) {
  ...
  Object.entries(tp)
    ...
    .forEach(({ name, rawCount, activeCount, activeTier, nextThreshold, isActive }) => {
      const pct          = Math.min((rawCount / nextThreshold) * 100, 100);
      const oddRemainder = rawCount - activeCount;
      const tierColors   = ['#6366f1','#a855f7','#eab308','#22c55e'];
      const tierIdx      = activeTier ? (SYNERGY_THRESHOLDS[name] || [2]).indexOf(activeTier) : -1;
      ...
      const pips         = thr.map(t => {
        ...
        return `<span class="syn-pip ${reached ? 'syn-pip--lit' : ''}" style="${reached ? 'background:' + activeClr : ''}"></span>`;
      }).join('');
        ...
        <div class="syn-info">
          <div class="syn-header">
            ...
            <div class="syn-count-group">
              ...
              <span class="syn-count-raw" style="color:${isActive ? activeClr : '#3f3f46'}">
                ${rawCount}${oddRemainder ? `<span class="syn-odd-hint">+1→${activeCount + 2}</span>` : ''}/${nextThreshold}
              </span>
            </div>
          </div>
          <div class="syn-bar-bg">
            <div class="syn-bar-fill" style="width:${pct}%;background:${isActive ? activeClr : 'rgba(255,255,255,0.12)'};${oddRemainder ? 'border-right:2px solid rgba(255,255,255,0.35)' : ''};"></div>
          </div>
          <div class="syn-pips">${pips}</div>
          ${name === 'Mortal Rival' && !mortalActive && rawCount >= 1 ? '<span class="syn-warning">⚠ Butuh Chou ATAU Valir, atau K.O.F ≥ 10</span>' : ''}
          ${oddRemainder ? `<span class="syn-odd-note">+1 hero lagi → aktif ${activeCount + 2}</span>` : ''}
        </div>`;
      el.appendChild(row);
    });
}
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ${equips.map((eq, i) => `
            <div style="display:flex;align-items:center;gap:14px;padding:13px 16px;border-radius:14px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);">
              <div style="width:28px;height:28px;border-radius:8px;flex-shrink:0;background:${['rgba(234,179,8,0.15)','rgba(148,163,184,0.12)','rgba(180,120,60,0.12)'][i]};border:1px solid ${['rgba(234,179,8,0.35)','rgba(148,163,184,0.3)','rgba(180,120,60,0.3)'][i]};display:flex;align-items:center;justify-content:center;font-size:15px;">${eq.icon}</div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:700;color:#e4e4e7;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${eq.name}</div>
                <div style="font-size:10px;color:#52525b;line-height:1.4;">${eq.desc}</div>
              </div>
...
function copyLineupToClipboard() {
  ...
  if (!playerHeroes.length) { showToast('Board kosong!', 'warn'); return; }
  const seen   = new Set();
  const unique = playerHeroes.filter(h => seen.has(h.name) ? false : (seen.add(h.name), true));
  const tp     = {};
  unique.forEach(h => {
  ...
  const lines      = Object.entries(tp)
    ...
    .join(', ');
  const text = `[MCGG Lineup] ${unique.map(h => h.name).join(', ')} — Sinergi: ${lines}`;
  // ── Clipboard write ──────────────────────────────────────────────────────────
  // Primary: Clipboard API (supported in all modern browsers over HTTPS).
  // Fallback: create a hidden <textarea>, focus it, and copy with the Selection
  //           API — NO execCommand. We use selection.toString() indirectly via
  //           a data URL so the text is in the selection buffer without execCommand.
  //           In practice, the Clipboard API almost always succeeds on modern
  //           browsers; the fallback is only needed on very old or sandboxed envs.
  async function writeToClipboard(text) {
    try {
      await navigator.clipboard.writeText(text);
      showToast('✓ Lineup disalin!', 'success');
    } catch {
      // Fallback: focus a hidden input and let the user copy manually if all else fails.
      // We avoid execCommand (deprecated) entirely.
      try {
        ...
        ta.focus();
        ta.select();
        // Use the modern Selection API path via document.execCommand only as
        // absolute last resort but wrapped so it never throws:
        const copied = document.execCommand?.('copy') ?? false;
        ta.remove();
        showToast(copied ? '✓ Lineup disalin!' : '⚠ Salin teks di bawah ini secara manual', copied ? 'success' : 'warn');
      } catch {
        showToast('⚠ Tidak bisa menyalin otomatis', 'warn');
      }
    }
  }
...
function bindEvents() {
  ...
  document.getElementById('share-lineup')?.addEventListener('click', copyLineupToClipboard);
}
...
function initOGMeta() {
  ...
  const board = document.getElementById('chess-board');
  if (board) {
    const mo = new MutationObserver(() => { clearTimeout(mo._t); mo._t = setTimeout(update, 150); });
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Builder hanya untuk user yang login
  requireAuth(() => {
    generateBoard();
    renderHeroPool();
    ...
    initOGMeta();
    document.getElementById('gl-select-1g')?.addEventListener('change', encodeToURL);
    document.getElementById('gl-select-5g')?.addEventListener('change', encodeToURL);
  }, 'untuk mengakses Lineup Builder');

  // Kalau user logout saat di halaman builder, tampilkan pesan
  onAuthChange(session => {
    if (!session) {
      const main = document.getElementById('main-content') || document.querySelector('.main-content');
      if (main) main.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:16px;color:#52525b;padding:40px;">
          <i class="ph-duotone ph-lock" style="font-size:48px;color:#3f3f46;"></i>
          <p style="font-size:16px;font-weight:700;color:#a1a1aa;">Login untuk mengakses Lineup Builder</p>
          <button onclick="document.getElementById('sidebar-login-btn').click()" class="btn btn--primary">
            <i class="ph-bold ph-sign-in"></i> Login Sekarang
          </button>
        </div>`;
    }
  });
